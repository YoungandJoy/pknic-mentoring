...
import {
  ...
  FUNCTIONS,
  MENTOR_SENIORITY,
  MENTEE_PRO_SENIORITY,
  MENTEE_PRO_FOCUS,
  MENTEE_PRO_CAREER_STAGE,
  MENTEE_PRO_NEEDS,
  PRO_TRACK_PREF,
  MENTOR_TOPICS,
  MENTEE_PAIN_POINTS,
...
const BigChoice = ({ active, onClick, children, sub }: any) => (
  <button
    type="button"
    onClick={onClick}
  ...
  >
    <div>{children}</div>
    {sub && (
      <div className={`text-[11px] mt-0.5 ${active ? "text-brand-bg/70" : "text-brand-textDim/70"}`}>
        {sub}
      </div>
    )}
  </button>
...
export default function SurveyForm() {
  ...
  const isMenteeStudent = isMentee && data.mentee_type === "student";
  const isMenteePro = isMentee && data.mentee_type === "professional";
  ...
  const validateStep = (): string | null => {
    if (step === 0) {
      ...
      if (isMentee && !data.mentee_type) return "멘티 유형(학생/직장인)을 선택해주세요";
    }
    if (step === 1) {
      if (isMentor) {
      ...
      if (isMenteePro) {
        if (!data.mentee_pro_company?.trim()) return "현재 회사를 입력해주세요";
        if (!data.mentee_pro_position?.trim()) return "현재 직무를 입력해주세요";
        if (!data.mentee_pro_seniority) return "경력 연차를 선택해주세요";
        if (!data.mentee_pro_career_stage) return "경력 단계를 선택해주세요";
        if (!(data.mentee_target_industries || []).length)
          return "관심 산업·도메인을 1개 이상 선택해주세요";
        if (!(data.mentee_pro_needs || []).length)
          return "멘토링 니즈를 1개 이상 선택해주세요";
      }
    }
  ...
  if (done) return <ResultPanel mine={data} />;
      ...
      <div className="rounded-2xl bg-brand-surface border border-brand-border p-6 md:p-8 space-y-6">
        ...
        {step === 0 && (
          <>
            ...
            <div className="grid grid-cols-2 gap-4">
              ...
              <Field label="도시">
                <input value={data.city || ""} onChange={(e) => update({ city: e.target.value })} placeholder="Seattle / Seoul / NYC" />
              </Field>
            </div>
            ...
            <Field label="참여 역할 *">
              <div className="grid grid-cols-2 gap-2">
                <BigChoice active={data.role === "mentor"} onClick={() => update({ role: "mentor", mentee_type: undefined })} sub="북미 거주 현직자가 시간을 내어 가이드 제공">
                  Mentor
                </BigChoice>
                <BigChoice active={data.role === "mentee"} onClick={() => update({ role: "mentee" })} sub="멘토와 1:1 매칭을 받고 싶은 분">
                  Mentee
                </BigChoice>
              </div>
            </Field>

            {/* Mentee 일 때만 학생/직장인 분기 */}
            {isMentee && (
              <Field label="멘티 유형 *" hint="둘 중 하나를 선택해주세요">
                <div className="grid grid-cols-2 gap-2">
                  <BigChoice
                    active={data.mentee_type === "student"}
                    onClick={() => update({ mentee_type: "student" })}
                    sub="학부·대학원 재학 중"
                  >
                    🎓 학생
                  </BigChoice>
                  <BigChoice
                    active={data.mentee_type === "professional"}
                    onClick={() => update({ mentee_type: "professional" })}
                    sub="현직 직장인 (이직·승진·창업 등 고민)"
                  >
                    💼 직장인
                  </BigChoice>
                </div>
            ...
            <Field label="멘토링 시 선호 언어">
              <div className="grid grid-cols-3 gap-2">
                {MENTORING_LANG.map((l) => (
                  <BigChoice key={l.v} active={data.mentoring_language_pref === l.v} onClick={() => update({ mentoring_language_pref: l.v })}>
                    {l.label}
                  </BigChoice>
                ))}
              </div>
            ...
            <Field label="어떻게 알게 되셨나요?">
              <select value={data.source || ""} onChange={(e) => update({ source: e.target.value })}>
                ...
                <option>지인 추천</option>
                <option>커뮤니티 (Slack/Discord/단톡방)</option>
                <option>이벤트/세미나</option>
                <option>인스타·SNS</option>
                <option>기타</option>
              </select>
            </Field>
          </>
        ...
        {step === 1 && isMentor && (
          <>
            ...
            <p className="text-xs text-brand-textDim -mt-3">북미 현직 시니어 — 멘티에게 노출될 핵심 프로필을 작성해주세요.</p>
            <div className="grid grid-cols-2 gap-4">
              <Field label="현재 회사 *">
                <input value={data.mentor_company || ""} onChange={(e) => update({ mentor_company: e.target.value })} placeholder="Google / Meta / Stripe ..." />
              </Field>
              <Field label="직무">
                <input value={data.mentor_position || ""} onChange={(e) => update({ mentor_position: e.target.value })} placeholder="Senior SWE / PM / Designer" />
              </Field>
            </div>
            <Field label="경력 연차 *">
              <div className="flex flex-wrap gap-2">
            ...
            <Field label="역할 유형">
              <div className="grid grid-cols-2 gap-2">
                {MGMT_TYPE.map((m) => (
                  <BigChoice key={m.v} active={data.mentor_management === m.v} onClick={() => update({ mentor_management: m.v })}>
                    {m.label}
                  </BigChoice>
                ))}
              </div>
            ...
            <Field label="다룰 수 있는 멘토링 주제">
              <div className="flex flex-wrap gap-2">
                {MENTOR_TOPICS.map((t) => (
                  <Pill key={t} active={(data.mentor_topics || []).includes(t)} onClick={() => toggle("mentor_topics", t)}>{t}</Pill>
                ))}
              </div>
            </Field>
            <Field label="이전 회사들 (자유 입력)">
              <input value={data.mentor_companies_history || ""} onChange={(e) => update({ mentor_companies_history: e.target.value })} placeholder="Apple → Stripe → Google" />
            </Field>
            <div className="grid grid-cols-2 gap-4">
              <Field label="북미 체류 연차">
                <select value={data.mentor_years_in_na || ""} onChange={(e) => update({ mentor_years_in_na: e.target.value })}>
                  <option value="">선택</option>
                  <option>1~3년</option>
                  <option>4~7년</option>
                  <option>8~15년</option>
              ...
              <Field label="한국 학력/배경">
                <select value={data.mentor_korean_origin || ""} onChange={(e) => update({ mentor_korean_origin: e.target.value })}>
                  <option value="">선택</option>
                  {KOREAN_ORIGIN.map((k) => (<option key={k.v} value={k.v}>{k.label}</option>))}
                </select>
              </Field>
            </div>
            <Field label="선호 멘티 유형">
              <div className="flex flex-wrap gap-2">
                {[
                  "북미 학부생", "북미 대학원생",
                  "한국 학부생", "한국 대학원생",
                  "주니어 직장인 (1~3년차)", "미드 직장인 (4~7년차)",
                  "Career switcher", "창업 준비",
                ].map((t) => (
                  <Pill key={t} active={(data.mentor_target_mentees || []).includes(t)} onClick={() => toggle("mentor_target_mentees", t)}>{t}</Pill>
                ))}
              </div>
            </Field>
        ...
        {step === 1 && isMenteeStudent && (
          <>
            <h2 className="text-xl font-bold">멘티 정보 — 학생</h2>
            <div className="grid grid-cols-2 gap-4">
              <Field label="소속 학교 *">
                <input value={data.mentee_school || ""} onChange={(e) => update({ mentee_school: e.target.value })} placeholder="UC Berkeley / 서울대 ..." />
              </Field>
              <Field label="학교 소재 국가">
                <select value={data.mentee_school_country || ""} onChange={(e) => update({ mentee_school_country: e.target.value as any })}>
                  <option value="">선택</option>
                  {COUNTRIES.map((c) => (<option key={c.v} value={c.v}>{c.label}</option>))}
                </select>
              </Field>
            ...
            <div className="grid grid-cols-2 gap-4">
              <Field label="전공 *">
                <input value={data.mentee_major || ""} onChange={(e) => update({ mentee_major: e.target.value })} placeholder="CS / Business / Bio ..." />
              </Field>
              <Field label="졸업 예정 연도">
                <input value={data.mentee_grad_year || ""} onChange={(e) => update({ mentee_grad_year: e.target.value })} placeholder="2027" />
              </Field>
            </div>
            <Field label="학위 과정">
              <div className="flex flex-wrap gap-2">
                {DEGREES.map((d) => (
                  <Pill key={d} active={data.mentee_degree === d} onClick={() => update({ mentee_degree: d })}>{d}</Pill>
                ))}
              </div>
            </Field>
            <Field label="신분 / 비자 (선택)" hint="매칭 멘토의 경험 매칭에 활용됩니다. 응답하지 않으셔도 됩니다.">
              <select value={data.visa_status || ""} onChange={(e) => update({ visa_status: e.target.value })}>
                <option value="">선택</option>
                {VISA_STATUS.map((v) => (<option key={v.v} value={v.v}>{v.label}</option>))}
              </select>
            </Field>
            <Field label="한국 학력/배경 (선택)" hint="비슷한 배경의 멘토 매칭에 활용됩니다.">
              <select value={data.mentee_korean_origin || ""} onChange={(e) => update({ mentee_korean_origin: e.target.value })}>
                <option value="">선택</option>
                {KOREAN_ORIGIN.map((k) => (<option key={k.v} value={k.v}>{k.label}</option>))}
              </select>
            </Field>
            <Field label="관심 산업군 *">
              <div className="flex flex-wrap gap-2">
                {INDUSTRIES.map((i) => (
                  <Pill key={i} active={(data.mentee_target_industries || []).includes(i)} onClick={() => toggle("mentee_target_industries", i)}>{i}</Pill>
                ))}
              </div>
            </Field>
            <Field label="관심 직무">
              <div className="flex flex-wrap gap-2">
                {FUNCTIONS.map((f) => (
                  <Pill key={f} active={(data.mentee_target_functions || []).includes(f)} onClick={() => toggle("mentee_target_functions", f)}>{f}</Pill>
                ))}
              </div>
            </Field>
            <Field label="타겟 마켓 *">
              <div className="grid grid-cols-3 gap-2">
                {[
                  ...
                  <BigChoice key={o.v} active={data.mentee_target_market === o.v} onClick={() => update({ mentee_target_market: o.v })}>
                    {o.l}
                  </BigChoice>
                ))}
              </div>
            ...
            <Field label="현재 커리어 단계">
              <select value={data.mentee_career_stage || ""} onChange={(e) => update({ mentee_career_stage: e.target.value })}>
                <option value="">선택</option>
            ...
            <Field label="가장 막히는 부분 (복수 선택)">
              <div className="flex flex-wrap gap-2">
                {MENTEE_PAIN_POINTS.map((p) => (
                  <Pill key={p} active={(data.mentee_pain_points || []).includes(p)} onClick={() => toggle("mentee_pain_points", p)}>{p}</Pill>
                ))}
              </div>
            </Field>
            ...
            <Field label="이력서 / 포트폴리오 URL (선택)">
              <input value={data.mentee_resume_url || ""} onChange={(e) => update({ mentee_resume_url: e.target.value })} placeholder="https://..." />
            </Field>
          </>
        )}
        ...
        {step === 1 && isMenteePro && (
          <>
            <h2 className="text-xl font-bold">멘티 정보 — 직장인</h2>
            <p className="text-xs text-brand-textDim -mt-3">
              현직 직장인이지만 다음 스텝(이직·승진·창업·학위 등)을 시니어 멘토와 함께 설계하고 싶은 분.
            </p>

            {/* ─ 섹션 1: 현재 위치 ─ */}
            <div className="text-[11px] uppercase tracking-wider font-bold text-brand-primary pt-2">
              ① 현재 위치
            </div>
            <div className="grid grid-cols-2 gap-4">
              <Field label="현재 회사 *">
                <input
                  value={data.mentee_pro_company || ""}
                  onChange={(e) => update({ mentee_pro_company: e.target.value })}
                  placeholder="회사명 (비공개로 두려면 '비공개')"
                />
              </Field>
              <Field label="현재 직책 *">
                <input
                  value={data.mentee_pro_position || ""}
                  onChange={(e) => update({ mentee_pro_position: e.target.value })}
                  placeholder="Senior SWE / Product Manager / Associate 등"
                />
              </Field>
            </div>
            <Field label="현재 직무 영역" hint="표준화된 직무 카테고리 — 매칭에 활용">
              <div className="flex flex-wrap gap-2">
                {FUNCTIONS.map((f) => (
                  <Pill
                    key={f}
                    active={data.mentee_pro_current_function === f}
                    onClick={() => update({ mentee_pro_current_function: f })}
                  >
                    {f}
                  </Pill>
                ))}
            ...
            <Field label="경력 연차 *">
              <div className="flex flex-wrap gap-2">
                {MENTEE_PRO_SENIORITY.map((s) => (
                  <Pill
                    key={s.v}
                    active={data.mentee_pro_seniority === s.v}
                    onClick={() => update({ mentee_pro_seniority: s.v })}
                  >
                    {s.label}
                  </Pill>
                ))}
            ...
            <Field label="경력 단계 *" hint="연차와 별개로 본인의 위치 카테고리를 선택해주세요">
              <div className="grid grid-cols-2 gap-2">
                {MENTEE_PRO_CAREER_STAGE.map((s) => (
                  <BigChoice
                    key={s.v}
                    active={data.mentee_pro_career_stage === s.v}
                    onClick={() => update({ mentee_pro_career_stage: s.v })}
                    sub={s.desc}
                  >
                    {s.label}
                  </BigChoice>
            ...
            <Field label="향후 트랙 선호">
              <div className="grid grid-cols-2 gap-2">
                {PRO_TRACK_PREF.map((t) => (
                  <BigChoice
                    key={t.v}
                    active={data.mentee_pro_management_track === t.v}
                    onClick={() => update({ mentee_pro_management_track: t.v })}
                  >
                    {t.label}
                  </BigChoice>
                ))}
            ...
            {/* ─ 섹션 2: 배경 ─ */}
            <div className="text-[11px] uppercase tracking-wider font-bold text-brand-primary pt-4 border-t border-brand-border">
              ② 배경
            </div>
            <Field label="신분 / 비자" hint="비자 sponsorship 경험 있는 멘토 매칭에 활용. 응답하지 않으셔도 됩니다.">
              <select value={data.visa_status || ""} onChange={(e) => update({ visa_status: e.target.value })}>
                <option value="">선택</option>
                {VISA_STATUS.map((v) => (<option key={v.v} value={v.v}>{v.label}</option>))}
              </select>
            </Field>
            <Field label="한국 학력/배경" hint="비슷한 배경의 멘토 매칭에 활용 (1.5세 / 2세 / 교환학생 / K-12 유학 등 다양한 케이스 포괄)">
              <select value={data.mentee_korean_origin || ""} onChange={(e) => update({ mentee_korean_origin: e.target.value })}>
                <option value="">선택</option>
                {KOREAN_ORIGIN.map((k) => (<option key={k.v} value={k.v}>{k.label}</option>))}
              </select>
            ...
            <Field label="가장 큰 카테고리 (1개)">
              <div className="flex flex-wrap gap-2">
                {MENTEE_PRO_FOCUS.map((f) => (
                  <Pill
                    key={f}
                    active={data.mentee_pro_focus === f}
                    onClick={() => update({ mentee_pro_focus: f })}
                  >
                    {f}
                  </Pill>
                ))}
            ...
            <Field label="구체적인 멘토링 니즈 *" hint="복수 선택 가능 — 매칭 우선순위에 활용됩니다">
              <div className="flex flex-wrap gap-1.5">
                {MENTEE_PRO_NEEDS.map((n) => (
                  <Pill
                    key={n}
                    active={(data.mentee_pro_needs || []).includes(n)}
                    onClick={() => toggle("mentee_pro_needs", n)}
                  >
                    {n}
                  </Pill>
                ))}
            ...
            <Field label="관심 산업·도메인 *" hint="이직·피봇 시 진입하고 싶은 산업">
              <div className="flex flex-wrap gap-2">
                {INDUSTRIES.map((i) => (
                  <Pill
                    key={i}
            ...
            <Field label="타겟 마켓 *">
              <div className="grid grid-cols-3 gap-2">
                {[
                  ...
                  <BigChoice
                    key={o.v}
                    active={data.mentee_target_market === o.v}
            ...
            <Field label="멘토에게 기대하는 것 (자유)" hint="500자 이내 — 구체적일수록 매칭이 정확해집니다">
              <textarea
                rows={4}
                value={data.mentee_expectations || ""}
                onChange={(e) => update({ mentee_expectations: e.target.value })}
                maxLength={500}
                placeholder="예: 5년차 PM에서 시니어 PM으로 가기 위한 성과 정의 / 한국 → 북미 이직 시 연봉·비자 협상 / 매니저 트랙 전환 후 첫 90일 가이드 등"
              />
            </Field>
          </>
        )}
        ...
        {step === 2 && (
          <>
            ...
            <Field label="매칭 시급도">
              <div className="grid grid-cols-2 gap-2">
                {URGENCY.map((u) => (
                  <BigChoice key={u.v} active={data.urgency === u.v} onClick={() => update({ urgency: u.v })}>
                    {u.label}
                  </BigChoice>
                ))}
              </div>
            ...
            <Field label="선호 진행 형식">
              <select value={data.match_format_pref || ""} onChange={(e) => update({ match_format_pref: e.target.value })}>
                <option value="">선택</option>
